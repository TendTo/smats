...
#define NEW_OPERATOR(class_name)                                                                           \
  static std::shared_ptr<const class_name> New() { return std::make_shared<const class_name>(Private{}); } \
  explicit class_name(Private)

#define NEW_OPERATOR_PARAMS(class_name, params, args)                                                                  \
  static std::shared_ptr<const class_name> New(params) { return std::make_shared<const class_name>(Private{}, args); } \
  class_name(Private, params)
...
class ExpressionCell : public std::enable_shared_from_this<ExpressionCell> {
 public:
  ...
  std::shared_ptr<ExpressionCell> ptr() { return shared_from_this(); }
  ...
  [[nodiscard]] virtual bool equal_to(const ExpressionCell& c) const = 0;
   ...
   */
  [[nodiscard]] virtual Expression evaluate_partial(const Environment& env) const = 0;
 ...
 protected:
  struct Private {};
  /**
   * Constructs ExpressionCell of kind @p k with @p is_poly and @p is_expanded, with a @p use_count of zero.
   * @param k kind of the expression
   * @param is_poly whether the expression is a polynomial
 ...
 private:
  ...
  const bool is_polynomial_{false};  ///< Whether the expression is a polynomial.
...
class UnaryExpressionCell : public ExpressionCell {
 public:
  ...
  [[nodiscard]] bool equal_to(const ExpressionCell& e) const override;
...
class ExpressionConstant : public ExpressionCell {
 public:
  ...
  [[nodiscard]] const T& value() const { return v_; }
  [[nodiscard]] Variables variables() const override;
  ...
  [[nodiscard]] T evaluate(const Environment& env) const override;
  [[nodiscard]] Expression expand() const override;
  [[nodiscard]] Expression evaluate_partial(const Environment& env) const override;
...
class ExpressionAdd : public ExpressionCell {
 public:
  NEW_OPERATOR_PARAMS(ExpressionAdd, PARAMS(mpq_class constant, std::map<Expression, mpq_class> expr_to_coeff_map),
  ...
  [[nodiscard]] bool equal_to(const ExpressionCell& e) const override;
  [[nodiscard]] bool less(const ExpressionCell& e) const override;
  [[nodiscard]] mpq_class evaluate(const Environment& env) const override;
  [[nodiscard]] Expression expand() const override;
  [[nodiscard]] Expression evaluate_partial(const Environment& env) const override;
  [[nodiscard]] Expression substitute(const Substitution& s) const override;
  [[nodiscard]] Expression differentiate(const Variable& x) const override;
  ...
  [[nodiscard]] mpq_class get_constant() const { return constant_; }
  /** Returns map from an expression to its coefficient. */
  [[nodiscard]] const std::map<Expression, mpq_class>& get_expr_to_coeff_map() const { return expr_to_coeff_map_; }

 private:
  std::ostream& DisplayTerm(std::ostream& os, bool print_plus, mpq_class coeff, const Expression& term) const;

  const mpq_class constant_{};
  const std::map<Expression, mpq_class> expr_to_coeff_map_;
};

/** Factory class to help build ExpressionAdd expressions. */
class ExpressionAddFactory {
 public:
  /** @constructor{ExpressionAddFactory} */
 ...
 private:
   ...
   */
  void AddTerm(mpq_class coeff, const Expression& term);
  /* Adds expr_to_coeff_map to this factory. It calls AddConstant and AddTerm
   * methods. */
  void AddMap(const std::map<Expression, mpq_class>& expr_to_coeff_map);

  bool is_expanded_{true};
  mpq_class constant_{0.0};
  std::map<Expression, mpq_class> expr_to_coeff_map_;
};
...
class ExpressionMul : public ExpressionCell {
 public:
  NEW_OPERATOR_PARAMS(ExpressionMul, PARAMS(mpq_class constant, std::map<Expression, Expression> base_to_exponent_map),
                      PARAMS(constant, base_to_exponent_map))
  ...
  [[nodiscard]] const std::map<Expression, Expression>& get_base_to_exponent_map() const {
...
class ExpressionMulFactory {
 public:
  ...
  explicit ExpressionMulFactory(const std::map<Variable, int>& base_to_exponent_map);
...
class ExpressionDiv : public BinaryExpressionCell {
 public:
  ...
  [[nodiscard]] Expression Expand() const override;
  [[nodiscard]] Expression EvaluatePartial(const Environment& env) const override;
  [[nodiscard]] Expression Substitute(const Substitution& s) const override;
  [[nodiscard]] Expression Differentiate(const Variable& x) const override;
  std::ostream& Display(std::ostream& os) const override;

 private:
  [[nodiscard]] mpq_class do_evaluate(mpq_class v1, mpq_class v2) const override;
};
...
class ExpressionLog : public UnaryExpressionCell {
 ...
 private:
  /* Throws std::runtime_exception if v ∉ [0, +oo). */
  static void check_domain(mpq_class v);
  [[nodiscard]] mpq_class do_evaluate(mpq_class v) const override;
};
...
class ExpressionPow : public BinaryExpressionCell {
 ...
 private:
  /* Throws std::runtime_exception if v1 is finite negative and v2 is finite
     non-integer. */
...
class ExpressionAsin : public UnaryExpressionCell {
 ...
 private:
  /* Throws std::runtime_exception if v ∉ [-1.0, +1.0]. */
  static void check_domain(mpq_class v);
  [[nodiscard]] mpq_class do_evaluate(mpq_class v) const override;
};
...
class ExpressionIfThenElse : public ExpressionCell {
 public:
  ...
  [[nodiscard]] const Formula& get_conditional_formula() const { return f_cond_; }
  /** Returns the 'then' expression. */
  [[nodiscard]] const Expression& get_then_expression() const { return e_then_; }
  /** Returns the 'else' expression. */
  [[nodiscard]] const Expression& get_else_expression() const { return e_else_; }
...
class ExpressionUninterpretedFunction : public ExpressionCell {
 public:
   ...
   * arguments.
   */
  ExpressionUninterpretedFunction(std::string name, std::vector<Expression> arguments);
  void hash(DelegatingHasher& hasher) const override;
  [[nodiscard]] Variables variables() const override;
  [[nodiscard]] bool equal_to(const ExpressionCell& e) const override;
  [[nodiscard]] bool less(const ExpressionCell& e) const override;
  ...
  [[nodiscard]] const std::vector<Expression>& get_arguments() const { return arguments_; }
...
bool is
    ...
    bool
    is_variable(const ExpressionCell& c);
