...
Expression NegateAddition(const Expression& e) {
  SMATS_ASSERT(is_addition(e), "Must be an addition");
...
Expression NegateMultiplication(const Expression& e) {
  SMATS_ASSERT(is_multiplication(e), "Must be a multiplication");
  return ExpressionMulFactory{to_multiplication(e)}.Negate().GetExpression();
}
}  // namespace

Expression::Expression(const mpq_class& constant) : cell_{ExpressionConstant::New(constant)} {}
Expression::Expression(const int constant) : cell_{ExpressionConstant::New(constant)} {}
Expression::Expression(const Variable& var) : cell_{ExpressionVar::New(var)} {}
Expression::Expression(const std::shared_ptr<const ExpressionCell>& cell) : cell_{cell} {}

ExpressionCell& Expression::mutable_cell() {
  SMATS_ASSERT(cell_.use_count() == 1, "There must be only one box pointing to the cell");
  return const_cast<ExpressionCell&>(cell());
}

Expression Expression::NaN() {
  static std::shared_ptr<const ExpressionNaN> const flyweight = ExpressionNaN::New();
  Expression result;
  result.cell_ = flyweight;
  return result;
}

Variables Expression::variables() const { return cell_->variables(); }

bool Expression::equal_to(const Expression& e) const {
  const ExpressionKind k1{kind()};
  const ExpressionKind k2{e.kind()};
  if (k1 != k2) return false;
  // Check structural equality.
  return cell().equal_to(e.cell());
}
...
bool Expression::is_polynomial() const { return cell().is_polynomial(); }
bool Expression::is_expanded() const { return cell().is_expanded(); }
Expression Expression::EvaluatePartial(const Environment& env) const { return cell().evaluate_partial(env); }
...
Expression Expression::Substitute(const Variable& var, const Expression& e) const {
  return cell().substitute({{var, e}});
}
Expression Expression::Substitute(const Substitution& s) const { return cell().substitute(s); }
Expression Expression::Differentiate(const Variable& x) const { return cell().differentiate(x); }
...
void Expression::SubImpl(const Expression& rhs) {
  ...
  SMATS_ASSERT(!(is_constant(lhs) && is_constant(rhs)));
  // x - y => x + (-y)
  lhs += -rhs;
}
...
void Expression::MulImpl(const Expression& rhs) {
  ...
  if (is_division(lhs) && is_division(rhs)) {
    lhs = (get_first_argument(lhs) * get_first_argument(rhs)) / (get_second_argument(lhs) * get_second_argument(rhs));
    return;
  }
...
class PrecisionGuard {
 public:
  PrecisionGuard(ostream* const os, const streamsize& new_precision) : os_{os}, original_precision_{os->precision()} {
...
ostream& operator<<(ostream& os, const Expression& e) {
  const PrecisionGuard precision_guard{&os, numeric_limits<double>::max_digits10};
  if (is_constant(e)) {
...
Expression atan2(const Expression& e1, const Expression& e2) {
  ...
  if (is_constant(e1) && is_constant(e2)) {
    return Expression{std::atan2(get_constant_value(e1), get_constant_value(e2))};
  }
...
Expression clamp(const Expression& v, const Expression& lo, const Expression& hi) {
  SMATS_ASSERT(!is_constant(lo) || !is_constant(hi) || (lo <= hi));
...
Expression if_then_else(const Formula& f_cond, const Expression& e_then, const Expression& e_else) {
  ...
  if (f_cond.equal_to(Formula::True())) {
    return e_then;
  }
  // simplification:: if(false, e1, e2) => e2
  if (f_cond.equal_to(Formula::False())) {
    return e_else;
  }
...
void DoEnumerateMultiIndex(const int order, const int num_vars, const int start_dim, const MultiIndex& base,
                           vector<MultiIndex>* const vec) {
  SMATS_ASSERT(order > 0);
  SMATS_ASSERT(start_dim >= 0);
  SMATS_ASSERT(base.size() == static_cast<size_t>(num_vars));
  if (order == 0) {
    return;
  }
  if (order == 1) {
...
Expression Derivative(Expression f, const MultiIndex& alpha, const Environment& a) {
  int i = 0;
...
Expression Exp(const vector<Expression>& terms, const MultiIndex& alpha) {
  SMATS_ASSERT(terms.size() == alpha.size());
...
void DoTaylorExpand(const Expression& f, const Environment& a, const vector<Expression>& terms, const int order,
                    const int num_vars, ExpressionAddFactory* const factory) {
  SMATS_ASSERT(order > 0);
  SMATS_ASSERT(terms.size() == static_cast<size_t>(num_vars));
  const vector<MultiIndex> multi_indices{EnumerateMultiIndex(order, num_vars)};
  for (const MultiIndex& alpha : multi_indices) {
    factory->AddExpression(Derivative(f, alpha, a) * Exp(terms, alpha) / FactorialProduct(alpha));
  }
}
}  // namespace
