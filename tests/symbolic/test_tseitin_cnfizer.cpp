...
using smats::Formula;
using smats::is_cnf;
using smats::TseitinCnfizer;
using smats::Variable;
using smats::drake::symbolic::ExpressionSubstitution;
using std::set;
using std::vector;
...
bool IsSatisfiable(const Formula &f) {
  ...
  SMATS_ASSERT(!vars.empty(), "Variables should not be empty.");
...
class TestTseitinCnfizer : public ::testing::Test {
 ...
 protected:
  ...
  TseitinCnfizer cnfizer_{config_};
};

TEST_F(TestTseitinCnfizer, CnfChecker) {
  SMATS_LOG_INIT_VERBOSITY(1);
